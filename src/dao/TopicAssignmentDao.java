...
public class TopicAssignmentDao {
    ...
    public int confirmChoice(int choiceId, int confirmerId, String college, String major,
        ...
        try {
            ...
            if (hasLegacyApprovedTopic(conn, snapshot.topicId)) {
                conn.rollback();
                return ERR_TOPIC_ASSIGNED;
            }
    ...
    public int manualAssign(int studentId, int topicId, int confirmerId, String college,
        ...
        try {
            ...
            String topicStatus;
            try (PreparedStatement ps = conn.prepareStatement(
                    "SELECT status FROM topics WHERE id=? AND college=? AND major=? FOR UPDATE")) {
                ps.setInt(1, topicId);
                ps.setString(2, college);
                ps.setString(3, major);
                try (ResultSet rs = ps.executeQuery()) {
            ...
            if (hasTopicAssignment(conn, topicId) || hasLegacyApprovedTopic(conn, topicId)) {
                conn.rollback();
                return ERR_TOPIC_ASSIGNED;
            }
            ...
            updateTopicSelected(conn, topicId);
            conn.commit();
            return assignmentId;
        } catch (Exception ex) {
            rollbackQuietly(conn);
