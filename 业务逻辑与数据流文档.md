...
| 密码存储方式 | BCrypt 哈希（兼容 MD5 旧数据自动登录） |
| 文档评阅权限 | 只有指导该学生的教师可以评阅其文档 |
| CSRF 防护 | 所有 POST .action 需携带 Session CSRF Token |
| 附件存储 | `uploads/{studentId}/` 相对路径，DownloadController 权限校验 |

---

## 十、Phase 2/3 已实现扩展功能

1. **答辩安排（defense_schedules）**：管理员 CRUD + Excel 批量导入；教师/学生查看
2. **站内信（messages）**：三角色收发消息；选题/评阅/答辩变更自动通知
3. **操作日志（operation_logs）**：关键操作审计 + 筛选分页
4. **数据统计**：ECharts 选题/文档/成绩分布 + Excel 导出
5. **文件上传**：multipart 上传 + 白名单校验 + 文档版本历史（document_versions）
6. **安全增强**：XSS 转义、CSRF、登录失败锁定、BCrypt 密码
