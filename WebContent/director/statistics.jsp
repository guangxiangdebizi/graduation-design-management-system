...
<%@ page import="bean.User,util.EscapeUtil" %>
<%
  request.setAttribute("pageTitle", "本专业项目统计");
  User loginUser = (User) session.getAttribute("loginUser");
  if (util.ScopeUtil.directorScope(loginUser) == null) {
...
<div class="alert alert-info py-2">
  当前统计范围：<%= EscapeUtil.html(loginUser.getCollegeName()) %> / <%= EscapeUtil.html(loginUser.getMajorName()) %>
</div>

<div class="d-flex justify-content-between align-items-center mb-3">
  <div></div>
  <a href="../director/export.action" class="btn btn-success btn-sm">导出本专业成绩 Excel</a>
</div>
...
(function() {
  ...
  try {
    fetch('../director/stats.action').then(function(r) {
